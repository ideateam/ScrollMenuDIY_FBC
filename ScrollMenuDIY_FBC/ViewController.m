...
- (void)viewDidLoad {
    ...
    //scrollMenu.menuW = 200;//根据需要是否需要进行用于自定义mySegmentedC的宽
    ...
    //初始化内容翻页
    [scrollMenu ScrollViewContent:CGRectMake(0, 0, SCREENW, SCREENH) andScrollDirection:FBScrollHorizontal andPagingEnabled:YES];
...
-(void)MenuButtonIsReallyClick:(UISegmentedControl *)SegmentedC{
    NSLog(@"v----selectedSegmentIndex = %ld",(long)SegmentedC.selectedSegmentIndex);
}
-(void)PlusButtonIsReallyClick:(UIButton *)button{
    NSLog(@"v----PlusButtonIsReallyClick =%ld",(long)button.tag);
}
-(void)PlusShowViewInsideButtonIsReallyClick:(UIButton *)button{
    NSLog(@"v----PlusShowViewInsideButtonIsReallyClick = %ld",(long)button.tag);
}
